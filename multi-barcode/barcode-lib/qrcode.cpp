...
BOOL CQR_Encode::EncodeSourceData(LPCSTR lpsSource, int ncLength, int nVerGroup)
{
	...
	{
		BYTE byMode;
		// 注释了对日文编码的优化，否则有可能把unicode判断为QR_MODE_KANJI，导致乱码
...
namespace z_qrcode
{
	HRESULT draw_image(HDC hdc,PQRCODEPARAM pQrCodeeParam,int &left,int &top,int code_c)
	{
		CQR_Encode qrEncode;
		BOOL bEncoded = qrEncode.EncodeData(
			pQrCodeeParam->nLevel,
			pQrCodeeParam->nVersion,
			pQrCodeeParam->bAutoExtent,
			pQrCodeeParam->nMaskingNo,
			pQrCodeeParam->lpsSource, NULL) ;
		...
		if ( NULL==hdc )
		{
			left = (qrEncode.m_nSymbleSize*code_c) + (QR_MARGIN * 2);
			top	 = left;
			return 0;
		}
		
		PatBlt(hdc, left, top, left+qrEncode.m_nSymbleSize*code_c+(QR_MARGIN*2),
			top+qrEncode.m_nSymbleSize*code_c+(QR_MARGIN*2), WHITENESS);//填充底色为白色
 
// 		SelectObject(hdc, (HBRUSH)GetStockObject(WHITE_BRUSH));
// 		SelectObject(hdc, (HPEN)GetStockObject(WHITE_PEN));
// 		Rectangle(hdc,left, top,
// 			left+qrEncode.m_nSymbleSize*code_c+(QR_MARGIN*2),
// 			top+qrEncode.m_nSymbleSize*code_c+(QR_MARGIN*2) );//填充底色为白色

		SelectObject(hdc, GetStockObject(BLACK_BRUSH));	// 选择画刷为黑色
		SelectObject(hdc, GetStockObject(BLACK_PEN));	// 选择画笔为黑色
		for (int i = 0; i < qrEncode.m_nSymbleSize; ++i)
		{
			for (int j = 0; j <qrEncode.m_nSymbleSize; ++j)
			{
				...
				{
					Rectangle(hdc,
						left+(i*code_c)+QR_MARGIN,
						top +(j*code_c)+QR_MARGIN,
						left+((i+1)*code_c)+QR_MARGIN,
						top +((j+1)*code_c)+QR_MARGIN );
				}
			}
		}
		return 0;
