...
{
	// 在hdc上绘制pdf417二维码
	// 当hdc为null时,left,top分别输出码字宽度和高度为code_x,code_y时,pdf二维码图像需要的图像宽度和高度.
	// code_x,code_y分别指定每个pdf417码字的宽度和高度.
...
// 		
// 		FillRect(hdc, &rc, (HBRUSH)GetStockObject(BLACK_BRUSH));//填充底色为黑色
		PatBlt(hdc, left, top, cols * code_x, rows * code_y,BLACKNESS);//填充底色为白色
		SelectObject(hdc, GetStockObject(WHITE_BRUSH));	//选择画刷为白色
		SelectObject(hdc, GetStockObject(WHITE_PEN));	//选择画笔为白色
...
// 	// 在hdc上绘制pdf417二维码
// 	// 当hdc为null时,ccol_,crow_分别输出二维码的列数和行数
// 	// 当hdc非null时,ccol_,crow_分别为要绘制pdf417二维码像素的宽度和高度,函数
// 	// 自动拉升图像宽度和高度ccol_,crow_指定的值.
// 	// ccol_,crow_应该大于二维码的列数和行数.否则图片会有问题.
